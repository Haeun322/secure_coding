...
상품별 1:1 메시지, 에스크로 결제/송금, 찜·후기, 신고·차단, 알림, 관리자 콘솔을
제공하며, 웹 애플리케이션에서 흔한 취약점(SQL Injection, XSS, CSRF, 접근 통제 미흡,
파일 업로드, 무차별 대입, 이중지불 등)을 막는 데 초점을 맞췄습니다.
...
| 상품 | 등록·수정·삭제, 이미지 업로드, 카테고리·지역, 목록·상세 |
| 검색 | 제목/설명 키워드 + 카테고리·지역 필터 + 정렬 + 페이지네이션 |
| 소통 | 상품별 1:1 메시지(실시간 갱신) |
| 결제·송금 | 지갑, 사용자 간 이체, 상품 결제(에스크로: 보류→확정→정산/취소 환불), 구매내역 |
| 찜·후기 | 관심상품(찜), 거래 후기 + 판매자 평판(별점) |
| 신고·차단 | 유저/상품 신고, 신고 누적 시 자동 숨김, 관리자 차단 |
| 알림 | 헤더 종+배지+드롭다운, 전체 알림 페이지, 메시지·거래 이벤트 실시간 반영 |
...
55개의 통합·단위 테스트가 기능과 보안 동작(CSRF, SQLi, XSS, 접근 통제/IDOR, 모더레이션
우회 방지, 오픈 리다이렉트, 계정 열거 방지, 에스크로 안전성, 스레드 10개 동시 결제/송금에서
이중지불 0건, 보류 상품 수정·삭제 차단, 차단 판매자 정산 거부, 알림 등)을 검증합니다.
`tests/test_app.py`가 기본 기능·보안을, `tests/test_hardening.py`가 정밀 검증에서 발견한
결함들의 회귀 방지를 담당합니다.
